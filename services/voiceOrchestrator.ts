...
import { Phase, View, Audience } from '../types';
...
class VoiceOrchestrator {
  ...
  private _updatePreferredVoice() {
    const voices = window.speechSynthesis.getVoices();
     if (voices.length > 0) {
      // Allow user to set a preferred voice
      ...
      if (!this.preferredVoice) {
          let selectedVoice = voices.find(voice => voice.name === "Microsoft Andrew Online (Natural) - English (United States)");
          if (!selectedVoice) selectedVoice = voices.find(voice => voice.lang === 'en-US' && voice.name.includes('Google'));
          if (!selectedVoice) selectedVoice = voices.find(voice => voice.lang === 'en-US' && voice.name.includes('David'));
          if (!selectedVoice) selectedVoice = voices.find(voice => voice.lang === 'en-US');
          this.preferredVoice = selectedVoice || null;
      }
      
      if(this.preferredVoice) {
          console.log(`Preferred voice set: ${this.preferredVoice.name}`);
      } else {
          console.warn(`Could not find a suitable en-US voice. The assistant will use the system's default voice.`);
      }
    }
  }
