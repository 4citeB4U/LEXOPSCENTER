...
export const handleShareBackup = async () => {
    CUE.tts.speak("Prepping your data for sharing.");
    try {
        if (!navigator.share) {
            CUE.tts.speak("Device sharing is not available on this browser. Try downloading the backup instead.");
            return;
        }

        const dataStr = await exportAllData();
        const dataBlob = new Blob([dataStr], { type: 'application/json' });
        const file = new File([dataBlob], `lex_backup_${new Date().toISOString().split('T')[0]}.json`, { type: 'application/json' });
        
        await navigator.share({
            title: 'LΞX Ops Center Backup',
            text: 'Here is my LΞX data backup.',
            files: [file],
        });
        localStorage.setItem('lexLastBackupTimestamp', new Date().toISOString());
    } catch (error) {
        ...
        if ((error as DOMException).name !== 'AbortError') {
            ...
            CUE.tts.speak("Sorry, I couldn't prepare the shareable backup.");
        }
    }
};