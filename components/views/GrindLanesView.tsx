...
const Lane: React.FC<{
    ...
    return (
        <div
            className={`w-full md:w-1/3 p-2 rounded-lg h-full flex flex-col ${isOver ? 'bg-slate-600/50' : ''} transition-colors`}
...
const GrindLanesView: React.FC<GrindLanesViewProps> = ({ assignments, onUpdateStatus, onEdit }) => {
    ...
    return (
        <div className="flex flex-col md:flex-row p-4 gap-4 h-full" onDragLeave={handleDragLeave}>
            {lanes.map(lane => (
                <div key={lane.id} className="w-full md:w-1/3 h-full flex flex-col" data-lane-status={lane.id}>
                    <Lane
                        title={lane.title}
                        status={lane.id}
                        color={lane.color}
